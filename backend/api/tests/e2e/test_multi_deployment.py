...
class TestMultiDeploymentIsolation:
    ...
    def test_user_isolation(
        self, client, mock_dynamodb, test_deployments, auth_headers
    ):
        ...
        with patch(
            "backend.api.repositories.user_repository.UserRepository.get_all_users",
            side_effect=lambda deployment_id, *args, **kwargs: deployment_a_users
            if deployment_id == deployment_a["id"]
            else deployment_b_users,
        ):
    ...
    def test_match_isolation(
        ...
        with patch(
            ...
            side_effect=lambda user_id, deployment_id: deployment_a_match
            if deployment_id == deployment_a["id"]
            else deployment_b_match,
        ):
            # User from deployment A
            response = client.get(
                "/matches/current",
    ...
    def test_cross_deployment_access_denied(
        self, client, mock_dynamodb, test_deployments, auth_headers
    ):
        ...
        with patch(
            ...
            side_effect=lambda match_id, deployment_id: deployment_a_match
            if deployment_id == deployment_a["id"] and match_id == deployment_a_match.id
            else None,
        ):
    ...
    def test_matching_isolation(self, client, mock_dynamodb, test_deployments):
        ...
        with patch(
            ...
            side_effect=lambda deployment_id, active_only=None, paused_only=None: [
                u
                for u in all_users
                if u.deployment_id == deployment_id and u.is_active and not u.is_paused
            ],
        ):
