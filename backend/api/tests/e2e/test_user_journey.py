...
class TestUserJourney:
    ...
    def test_full_user_journey(self, client, mock_dynamodb, mock_ses, test_users, auth_headers):
        """
        ...
        for user in test_users:
            ...
            with patch('backend.api.repositories.user_repository.UserRepository.create_user',
        ...
        with patch('backend.api.repositories.user_repository.UserRepository.update_user',
                  return_value=User(**test_users[0], 
                                   deployment_id="test-deployment",
                                   is_active=True,
                                   is_paused=False,
        ...
        match = Match(
            id="test-match-1",
            participants=[
                {"id": test_users[0]["id"], "name": test_users[0]["name"], "email": test_users[0]["email"]},
                {"id": test_users[1]["id"], "name": test_users[1]["name"], "email": test_users[1]["email"]}
            ],
        ...
        with patch('backend.api.services.matching_service.MatchingService.create_matches',
                  return_value=[match]):
        ...
        with patch('backend.api.repositories.match_repository.MatchRepository.get_match',
                  return_value=match):
            with patch('backend.api.repositories.match_repository.MatchRepository.update_match',
        ...
        with patch('backend.api.repositories.match_repository.MatchRepository.get_match',
            ...
            with patch('backend.api.repositories.match_repository.MatchRepository.update_match',
                ...
                assert response.json()["feedback"][0]["comments"] == "Great conversation!"
        
        # Step 7: Check match history
        with patch('backend.api.repositories.match_repository.MatchRepository.get_user_matches',
                  return_value=[updated_match]):
