...
def test_oauth_login_amazon_sso(client):
    ...
    with patch("backend.api.auth.oauth.generate_authorization_url") as mock_gen_url:
        ...
        assert response.headers["location"] == "https://sso.amazon.com/oauth2/authorize?mock=true"
...
def test_oauth_login_google(client):
    ...
    with patch("backend.api.auth.oauth.generate_authorization_url") as mock_gen_url:
        # Set up the mock to return a known URL
        mock_gen_url.return_value = "https://accounts.google.com/o/oauth2/auth?mock=true"
        ...
        # Verify the response is a redirect
        assert response.status_code == 307
        assert response.headers["location"] == "https://accounts.google.com/o/oauth2/auth?mock=true"
...
def test_oauth_callback_amazon_sso(client):
    ...
    with patch("backend.api.main.handle_oauth_callback") as mock_handle_callback, \
         patch("backend.api.main.get_deployment_id_from_state") as mock_get_deployment_id:
        ...
        response = client.get(f"/auth/amazon-sso/callback?code=test-code&state={test_state}")
        ...
        mock_handle_callback.assert_called_once_with("amazon-sso", "test-code", test_state)
...
def test_oauth_callback_amazon(client):
    ...
    with patch("backend.api.main.handle_oauth_callback") as mock_handle_callback, \
        ...
        # Make the request to the OAuth callback endpoint
        response = client.get(f"/auth/amazon/callback?code=test-code&state={test_state}")
...
def test_oauth_callback_google(client):
    ...
    with patch("backend.api.main.handle_oauth_callback") as mock_handle_callback, \
        ...
        # Make the request to the OAuth callback endpoint
        response = client.get(f"/auth/google/callback?code=test-code&state={test_state}")
...
def test_oauth_callback_error(client):
    ...
    with patch("backend.api.main.handle_oauth_callback") as mock_handle_callback:
        ...
        # Make the request to the OAuth callback endpoint
        response = client.get("/auth/google/callback?code=test-code&state=invalid-state")
        ...
        # Verify the handle_oauth_callback function was called with the correct arguments
        mock_handle_callback.assert_called_once_with("google", "test-code", "invalid-state")


def test_generate_authorization_url():
    """Test generating an authorization URL."""
    # Call the function directly
    url = generate_authorization_url("google", "http://localhost:8000", "test-deployment")
...
async def test_handle_oauth_callback():
    ...
    with patch("httpx.AsyncClient") as mock_client:
        ...
        assert userinfo_call[1]["headers"]["Authorization"] == "Bearer mock-access-token"