...
class NotificationChannel(ABC):
    ...
    @abstractmethod
    async def send_notification(self, user: User, match: Match, other_participants: List[User]) -> bool:
        """
...
class SlackChannel(NotificationChannel):
    ...
    def is_available_for_user(self, user: User) -> bool:
        ...
        """
        return (
            user.notification_prefs and
            user.notification_prefs.slack and
            user.notification_prefs.slack_webhook
        )
...
class TelegramChannel(NotificationChannel):
    ...
    async def send_notification(self, user: User, match: Match, other_participants: List[User]) -> bool:
        ...
        try:
            ...
            if not user.notification_prefs or not user.notification_prefs.telegram_chat_id:
                logger.error(f"User {user.id} has no Telegram chat ID configured")
                return False
            
            # Create message text
            message_text = f"*Virtual Coffee Match*\n\n"
            message_text += f"Hello {user.name},\n\n"
            message_text += f"You've been matched for a virtual coffee meeting!\n\n"
            message_text += f"*Your match details:*\n"
    ...
    def is_available_for_user(self, user: User) -> bool:
        ...
        return (
            user.notification_prefs and
            user.notification_prefs.telegram and
            user.notification_prefs.telegram_chat_id
        )
...
class SignalChannel(NotificationChannel):
    ...
    async def send_notification(self, user: User, match: Match, other_participants: List[User]) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Failed to send Signal notification: {response.status_code} {response.text}")
                return False
        except Exception as e:
    ...
    def is_available_for_user(self, user: User) -> bool:
        ...
        return (
            user.notification_prefs and
            user.notification_prefs.signal and
            user.notification_prefs.signal_number
        )