...
class UserRepository(BaseRepository[User]):
    ...
    async def get_all(self, filter_params: Optional[Dict[str, Any]] = None) -> List[User]:
        """
            ...
            for key, value in user_update.items():
                ...
                if key == 'preferences' and value:
                    for pref_key, pref_value in value.items():
                        update_expressions.append(f'#preferences.#{pref_key} = :preferences_{pref_key}')
                        expression_names[f'#preferences'] = 'preferences'
