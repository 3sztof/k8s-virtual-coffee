...
def check_argocd_application(instance):
    ...
    result = run_command(
        f"kubectl get application.argoproj.io/virtual-coffee-{instance} -n argocd -o json"
    )
...
def check_dynamodb_resources(instance):
    ...
    result = run_command(
        f"kubectl get virtualcoffeedynamodbclaim.virtualcoffee.io/{instance}-dynamodb -n {instance} -o json"
    )
    ...
    try:
        ...
        if ready:
            ...
            if aws_result.returncode == 0:
                ...
                print_warning(
                    "This may be expected if running in a development environment"
                )
...
def check_api_health(instance):
    ...
    try:
        ...
        if (
            ...
            or "healthy" not in health_result.stdout.lower()
        ):
...
def test_rollback_procedure(instance):
    ...
    result = run_command(
        f"kubectl get application.argoproj.io/virtual-coffee-{instance} -n argocd -o jsonpath='{{.status.sync.revision}}'"
    )
    ...
    patch_result = run_command(
        f'kubectl patch application.argoproj.io/virtual-coffee-{instance} -n argocd --type merge -p \'{{"spec":{{"source":{{"path":"non-existent-path"}}}}}}\' --record'
    )
    ...
    sync_result = run_command(
        f"kubectl get application.argoproj.io/virtual-coffee-{instance} -n argocd -o jsonpath='{{.status.sync.status}}'"
    )
    ...
    rollback_result = run_command(
        f'kubectl patch application.argoproj.io/virtual-coffee-{instance} -n argocd --type merge -p \'{{"spec":{{"source":{{"path":"k8s/overlays/dev"}}}}}}\' --record'
    )
    ...
    if final_sync_result.stdout.strip() == "Synced":
        ...
        print_error(
            f"Application failed to sync after rollback: {final_sync_result.stdout.strip()}"
        )
        return False
...
def test_resource_recovery(instance):
    ...
    result = run_command(
        f"kubectl get deployment -n {instance} -o jsonpath='{{.items[0].metadata.name}}'"
    )
    ...
    delete_result = run_command(
        f"kubectl delete deployment {deployment_name} -n {instance}"
    )
    ...
    check_result = run_command(
        f"kubectl get deployment {deployment_name} -n {instance} --ignore-not-found"
    )
    ...
    if restored:
        print_success(
            f"Deployment {deployment_name} was successfully restored by ArgoCD"
        )
        ...
        ready_result = run_command(
            f"kubectl rollout status deployment/{deployment_name} -n {instance} --timeout=60s"
        )
        ...
        if ready_result.returncode == 0:
            ...
            print_warning(
                f"Deployment {deployment_name} was restored but may not be ready"
            )
            return True  # Still return True as the resource was recovered
    else:
        print_error(
            f"Deployment {deployment_name} was not restored within the timeout period"
        )
        return False
            ...
            if req_id in endpoints_to_check:
                ...
                for endpoint in endpoints:
                    if endpoint in docs_result.stdout:
                        print_success(
                            f"Endpoint {endpoint} found for requirement {req_id}"
                        )
                    else:
                        print_warning(
                            f"Endpoint {endpoint} not found for requirement {req_id}"
                        )
                ...
                if req_id == "5":
                    ...
                    if isolation_result.returncode == 0:
                        print_success(
                            f"Requirement {req_id} verified: Namespace isolation"
                        )
                    else:
                        print_warning(
                            f"Requirement {req_id} not verified: Namespace isolation issue"
                        )

                elif req_id == "6":
                    # Check ArgoCD and Crossplane integration
                    argocd_result = run_command(
                        f"kubectl get application.argoproj.io/virtual-coffee-{instance} -n argocd"
                    )
                    crossplane_result = run_command(
                        f"kubectl get virtualcoffeedynamodbclaim.virtualcoffee.io/{instance}-dynamodb -n {instance}"
                    )

                    if (
                        argocd_result.returncode == 0
                        and crossplane_result.returncode == 0
                    ):
                        print_success(
                            f"Requirement {req_id} verified: ArgoCD and Crossplane integration"
                        )
                    else:
                        print_warning(
                            f"Requirement {req_id} not fully verified: ArgoCD or Crossplane issue"
                        )

                elif req_id == "7":
                    # Check frontend for Cloudscape components
                    print_info(
                        f"Requirement {req_id}: Manual verification required for Cloudscape Design System"
                    )

                elif req_id == "15":
                    # Check Makefile targets
                    makefile_result = run_command(
                        "grep -c 'deploy-instance\|destroy-instance' Makefile"
                    )
                    if (
                        makefile_result.returncode == 0
                        and int(makefile_result.stdout.strip()) > 0
                    ):
                        print_success(
                            f"Requirement {req_id} verified: Makefile automation targets"
                        )
                    else:
                        print_warning(
                            f"Requirement {req_id} not verified: Makefile automation targets issue"
                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate a Virtual Coffee Platform deployment"
    )
    parser.add_argument("--instance", required=True, help="Instance name to validate")
    parser.add_argument(
        "--validate-rollback", action="store_true", help="Run rollback validation tests"
    )
