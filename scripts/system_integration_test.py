...
def update_user_preferences(api_url, token, preferences):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_error(
                f"Failed to update preferences: {response.status_code} {response.text}"
            )
            return None
    except Exception as e:
...
def run_matching_algorithm(api_url, deployment_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_error(
                f"Failed to run matching: {response.status_code} {response.text}"
            )
            return None
    except Exception as e:
...
def get_current_match(api_url, token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_error(
                f"Failed to get current match: {response.status_code} {response.text}"
            )
            return None
    except Exception as e:
...
def update_match_status(api_url, token, match_id, status):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_error(
                f"Failed to update match status: {response.status_code} {response.text}"
            )
            return None
    except Exception as e:
...
def submit_match_feedback(api_url, token, match_id, rating, comments):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_error(
                f"Failed to submit feedback: {response.status_code} {response.text}"
            )
            return None
    except Exception as e:
...
def get_match_history(api_url, token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_error(
                f"Failed to get match history: {response.status_code} {response.text}"
            )
            return None
    except Exception as e:
...
def test_user_journey(api_url, deployment_id):
    ...
    updated_user = update_user_preferences(
        api_url, tokens[registered_users[0]["id"]], updated_preferences
    )
    if not updated_user:
    ...
    updated_match = update_match_status(
        api_url, tokens[registered_users[0]["id"]], match["id"], "completed"
    )
...
def test_multi_tenant_isolation(api_url_a, api_url_b, deployment_a, deployment_b):
    ...
    for participant_id in participant_ids_a:
        if participant_id not in user_ids_a:
            print_error(
                f"User from another deployment found in deployment A match: {participant_id}"
            )
            return False
    ...
    for participant_id in participant_ids_b:
        if participant_id not in user_ids_b:
            print_error(
                f"User from another deployment found in deployment B match: {participant_id}"
            )
            return False
...
def test_disaster_recovery(instance):
    ...
    result = run_command(
        f"kubectl get pods -n {instance} -l app=virtual-coffee-api -o jsonpath='{{.items[0].metadata.name}}'"
    )
    ...
    check_result = run_command(
        f"kubectl get pods -n {instance} -l app=virtual-coffee-api"
    )
    ...
    patch_result = run_command(
        f"kubectl patch configmap -n {instance} virtual-coffee-config --type merge -p '{json.dumps(config_patch)}'"
    )
    ...
    sync_result = run_command(
        f"kubectl annotate application.argoproj.io/virtual-coffee-{instance} -n argocd argocd.argoproj.io/refresh=hard"
    )
    ...
    check_result = run_command(
        f"kubectl get configmap -n {instance} virtual-coffee-config -o yaml"
    )
    if check_result.returncode != 0:
        print_error("Failed to get ConfigMap")
        return False

    if "TEST_KEY: test-value-" in check_result.stdout:
    ...
    restore_result = run_command(
        f"make restore-instance INSTANCE={instance} BACKUP_FILE={backup_file} DRY_RUN=true"
    )
...
def verify_mvp_requirements(instance):
    ...
    validation_result = run_command(
        f"python scripts/validate_deployment.py --instance {instance}"
    )
    ...
    for req in requirements:
        if (
            f"Requirement {req['id']}" in validation_result.stdout
            and "verified" in validation_result.stdout
        ):
            print_success(f"Requirement {req['id']}: {req['description']} - VERIFIED")
        else:
            print_warning(
                f"Requirement {req['id']}: {req['description']} - NOT VERIFIED"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run system integration tests for the Virtual Coffee Platform"
    )
    parser.add_argument(
        "--instance-a", required=True, help="First instance name to test"
    )
    parser.add_argument(
        "--instance-b",
        required=True,
        help="Second instance name to test (for multi-tenant tests)",
    )
    parser.add_argument(
        "--skip-user-journey", action="store_true", help="Skip user journey tests"
    )
    parser.add_argument(
        "--skip-multi-tenant",
        action="store_true",
        help="Skip multi-tenant isolation tests",
    )
    parser.add_argument(
        "--skip-disaster-recovery",
        action="store_true",
        help="Skip disaster recovery tests",
    )
    parser.add_argument(
        "--skip-mvp-verification",
        action="store_true",
        help="Skip MVP requirement verification",
    )
    ...
    api_port_forward_a = setup_port_forwarding(
        instance_a, "virtual-coffee-api", 8001, 80
    )
    frontend_port_forward_a = setup_port_forwarding(
        instance_a, "virtual-coffee-frontend", 3001, 80
    )

    # Set up port forwarding for instance B
    api_port_forward_b = setup_port_forwarding(
        instance_b, "virtual-coffee-api", 8002, 80
    )
    frontend_port_forward_b = setup_port_forwarding(
        instance_b, "virtual-coffee-frontend", 3002, 80
    )
    ...
    try:
        ...
        if not args.skip_multi_tenant:
            results["multi_tenant"] = test_multi_tenant_isolation(
                api_url_a, api_url_b, instance_a, instance_b
            )
        else:
